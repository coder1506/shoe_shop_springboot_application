	...
	public boolean save(MultipartFile image,SlideEntity slide) throws IllegalStateException, IOException {
		if(slide.getId() != null) {// if this slide had in db
			...
			if(!image.isEmpty()) { //if admin uploads slide image
				new File("C:\\Users\\ADMIN\\Desktop\\shoe_shop_springboot_application\\com.shoes_shop.duc\\uploads\\slides\\" + oldSlide.getImage()).delete();
			}
			else {
		...
		if(!image.isEmpty()) {
			image.transferTo(new File("C:\\Users\\ADMIN\\Desktop\\shoe_shop_springboot_application\\com.shoes_shop.duc\\uploads\\slides\\"+image.getOriginalFilename())); 
